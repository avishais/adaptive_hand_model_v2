...
class Covariance(object):
	...
	def ml_estimate(self,x,t):
		...
		# print "Initial theta: ", theta_start
...
class GaussianCovariance(Covariance):
	...
	def __call__(self,xi,xj,theta):
		...
		# vt = np.exp(theta[1])
		# w = np.exp(theta[2:])
		v, vt, w = self.theta_components(theta)
		#Winv = np.diag(w)
	...
	def theta_components(self, theta):
		v = np.exp(theta[0])
		vt = np.exp(theta[1])
		w = np.exp(theta[2:])
		# v = theta[0]
	...
	def _d_cov_matrix_d_theta(self,x,theta,j):
		# vt = np.exp(theta[1])
		_, vt, _ = self.theta_components(theta)
	...
	def _d_cov_matrix_d_x(self,x,theta,i,dim,Cov= None):
		...
		# w =np.exp( theta[2:])
		v, _, w = self.theta_components(theta)
		#Winv = np.diag(w)
